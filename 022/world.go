...
func (value *Vector) Equal(other *Vector) bool {
...
type Plane struct {
	// plane: normal.x * x + normal.y * y + normal.z * z = d
	normal Vector
	d      int64
}
...
// Is the point in front of the plane, according to the normal direction. Inclusive.
func (value *Plane) InFront(x int64, y int64, z int64) bool {
	return Dot(x, y, z, value.normal.x, value.normal.y, value.normal.z) >= value.d
}
...
func (zone *Zone) Inside(x int64, y int64, z int64) bool {
	for _, volume := range zone.volumes {
...
func (value *World) FindZoneId(x int64, y int64, z int64, zone_id *uint32) bool {
	for _, zone := range value.zones {
		fmt.Printf("id=%d\n", zone.id)
		if zone.Inside(x, y, z) {
			*zone_id = zone.id
			return true
		}
	}
